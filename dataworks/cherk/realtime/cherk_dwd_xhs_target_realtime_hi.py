# DataWorks PyODPS 节点：按项目校验定向实时数据完整性
import json
from odps import options
options.sql.settings = {'odps.sql.type.system.odps2': 'true'}

ds = args['bizdate']
base_table = 'dim_xhs_target_df'
cherk_table = 'dwd_xhs_target_realtime_hi'
cherk_source = '定向层级实时报表'

count_sql = f"""
SELECT a.project_id,
       MAX(p.project_name) AS project_name,
       COUNT(DISTINCT a.target_id) AS base_count,
       COUNT(DISTINCT b.target_id) AS cherk_count
FROM {base_table} a
LEFT JOIN {cherk_table} b
  ON a.target_id = b.target_id AND b.ds = '{ds}'
...
for row in projects:
    ...
    if base_count > cherk_count:
        ...
        with o.execute_sql(miss_sql).open_reader() as r:
            missing = [row2['target_id'] for row2 in r]
