...
SELECT /*+ MAPJOIN(attr) */
    ...
    CAST(p.ds AS BIGINT)                                                          AS dt,

    -- ============ 金额指标（直接取数） ============
    CAST(COALESCE(e.fee, 0) AS DECIMAL(20,6))                                    AS fee,
    CAST(COALESCE(e.ad_fee, 0) AS DECIMAL(20,6))                                 AS ad_fee,
    CAST(COALESCE(e.kol_price, 0) AS BIGINT)                                     AS kols_fee,

    -- ============ 笔记数 ============
    CAST(COALESCE(e.note_cnt, 0) - COALESCE(s.note_cnt, 0) AS BIGINT)            AS note_count,
    ...
    CAST(CASE WHEN (COALESCE(e.impression, 0) - COALESCE(s.impression, 0)) > 0
         THEN COALESCE(e.fee, 0) * 1000.0 / (COALESCE(e.impression, 0) - COALESCE(s.impression, 0))
         ELSE 0 END AS DECIMAL(20,6))                                             AS cpm,
    -- CPE
    CAST(CASE WHEN (COALESCE(e.interaction, 0) - COALESCE(s.interaction, 0)) > 0
         THEN COALESCE(e.fee, 0) * 1.0 / (COALESCE(e.interaction, 0) - COALESCE(s.interaction, 0))
         ELSE 0 END AS DECIMAL(20,6))                                             AS cpe,
    -- CPC
    CAST(CASE WHEN (COALESCE(e.click, 0) - COALESCE(s.click, 0)) > 0
         THEN COALESCE(e.fee, 0) * 1.0 / (COALESCE(e.click, 0) - COALESCE(s.click, 0))
    ...
    CAST(CASE WHEN attr.attribution_period = '15' AND (COALESCE(e.15d_enter_shop_uv, 0) - COALESCE(s.15d_enter_shop_uv, 0)) > 0
         THEN COALESCE(e.fee, 0) * 1.0 / (COALESCE(e.15d_enter_shop_uv, 0) - COALESCE(s.15d_enter_shop_uv, 0))
         WHEN attr.attribution_period = '30' AND (COALESCE(e.30d_enter_shop_uv, 0) - COALESCE(s.30d_enter_shop_uv, 0)) > 0
         THEN COALESCE(e.fee, 0) * 1.0 / (COALESCE(e.30d_enter_shop_uv, 0) - COALESCE(s.30d_enter_shop_uv, 0))
    ...
    CAST(CASE WHEN attr.attribution_period = '15' AND (COALESCE(e.15d_shop_new_customer_uv, 0) - COALESCE(s.15d_shop_new_customer_uv, 0)) > 0
         THEN COALESCE(e.fee, 0) * 1.0 / (COALESCE(e.15d_shop_new_customer_uv, 0) - COALESCE(s.15d_shop_new_customer_uv, 0))
         WHEN attr.attribution_period = '30' AND (COALESCE(e.30d_shop_new_customer_uv, 0) - COALESCE(s.30d_shop_new_customer_uv, 0)) > 0
         THEN COALESCE(e.fee, 0) * 1.0 / (COALESCE(e.30d_shop_new_customer_uv, 0) - COALESCE(s.30d_shop_new_customer_uv, 0))
         ELSE 0 END AS DECIMAL(20,6))                                             AS cac,
    -- 全店ROI
    CAST(CASE WHEN COALESCE(e.fee, 0) > 0
         THEN CASE WHEN attr.attribution_period = '15' THEN (COALESCE(e.15d_shop_order_gmv, 0) - COALESCE(s.15d_shop_order_gmv, 0)) * 1.0 / COALESCE(e.fee, 0)
                   ELSE (COALESCE(e.30d_shop_order_gmv, 0) - COALESCE(s.30d_shop_order_gmv, 0)) * 1.0 / COALESCE(e.fee, 0) END
         ELSE 0 END AS DECIMAL(20,6))                                             AS roi,
    -- 单品ROI
    CAST(CASE WHEN COALESCE(e.fee, 0) > 0
         THEN CASE WHEN attr.attribution_period = '15' THEN (COALESCE(e.15d_task_product_gmv, 0) - COALESCE(s.15d_task_product_gmv, 0)) * 1.0 / COALESCE(e.fee, 0)
                   ELSE (COALESCE(e.30d_task_product_gmv, 0) - COALESCE(s.30d_task_product_gmv, 0)) * 1.0 / COALESCE(e.fee, 0) END
...
LEFT JOIN dws_xhs_project_cum s
    ...
    AND s.ds = p.ds
WHERE p.ds >= TO_CHAR(DATEADD(TO_DATE('${bizdate}', 'yyyymmdd'), -29, 'dd'), 'yyyymmdd')
  AND p.ds <= '${bizdate}'
;
