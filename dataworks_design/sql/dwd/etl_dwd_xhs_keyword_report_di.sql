-- ============================================================
-- ETL: ODS → DWD 关键词层日离线明细
-- 源表: ods_xhs_keyword_report_di
-- 目标表: dwd_xhs_keyword_report_di
-- ============================================================

INSERT OVERWRITE TABLE dwd_xhs_keyword_report_di PARTITION (ds='${bizdate}')
SELECT
    -- 标识字段
    a.advertiser_id,                                                                           -- 投放账号ID
    GET_JSON_OBJECT(a.raw_data, '$.keyword')                             AS keyword,            -- 关键词
    GET_JSON_OBJECT(a.raw_data, '$.keyword_id')                          AS keyword_id,         -- 关键词ID
    ...
    -- 特有字段
    CAST(GET_JSON_OBJECT(a.raw_data, '$.word_avg_location') AS DECIMAL(18,4)) AS word_avg_location, -- 关键词平均排名
    -- 时间字段
    a.dt,                                                                                      -- 数据时间
    -- 核心指标
    ...
    REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(
        CONCAT('{',
            ...
            '"add_wechat_count":', COALESCE(GET_JSON_OBJECT(a.raw_data, '$.add_wechat_count'), '0'), ',',
            '"add_wechat_suc_count":', COALESCE(GET_JSON_OBJECT(a.raw_data, '$.add_wechat_suc_count'), '0'), ',',
            '"wechat_talk_count":', COALESCE(GET_JSON_OBJECT(a.raw_data, '$.wechat_talk_count'), '0'),
        '}'),
        '"[^"]+":0(\\.\\d+)?,?', ''),
        ',}', '}'),
        '\\{,', '{')
    ...
    REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(
        CONCAT('{',
            ...
            '"shop_poi_click_num":', COALESCE(GET_JSON_OBJECT(a.raw_data, '$.shop_poi_click_num'), '0'), ',',
            '"shop_poi_page_navigate_click":', COALESCE(GET_JSON_OBJECT(a.raw_data, '$.shop_poi_page_navigate_click'), '0'), ',',
            '"shop_poi_page_pv":', COALESCE(GET_JSON_OBJECT(a.raw_data, '$.shop_poi_page_pv'), '0'),
        '}'),
        '"[^"]+":0(\\.\\d+)?,?', ''),
        ',}', '}'),
        '\\{,', '{')
    ...
    -- 系统字段
    GETDATE() AS etl_time
FROM ods_xhs_keyword_report_di a
WHERE a.ds = '${bizdate}'
;
