-- ============================================================
-- ETL: ODS → DWD 转化明细日增量
-- 源表: ods_xhs_grass_ad_conversion_di, ods_xhs_grass_bycontent_conversion_di
...
SELECT
    ...
    -- 时间字段
    b.dt,                                                                                    -- 数据时间
    -- 站外行为指标 (来源: ods_xhs_grass_ad_conversion_di)
    COALESCE(CAST(a.offsite_active_uv AS BIGINT), 0)                 AS ad_offsite_active_uv,       -- 站外活跃行为UV
    COALESCE(CAST(a.offsite_task_cost AS DECIMAL(18,2)), 0)             AS ad_offsite_task_cost,       -- 站外任务期消费
    COALESCE(CAST(a.offsite_task_read_uv AS BIGINT), 0)              AS ad_offsite_task_read_uv,    -- 站外任务期阅读UV
    COALESCE(CAST(a.offsite_active_uv_dedup AS BIGINT), 0)           AS ad_offsite_active_uv_dedup, -- 站外活跃行为UV(去重)
    COALESCE(CAST(a.offsite_task_cost_dedup AS DECIMAL(18,2)), 0)      AS ad_offsite_task_cost_dedup, -- 站外任务期消费(去重)
    COALESCE(CAST(a.offsite_task_read_uv_dedup AS BIGINT), 0)        AS ad_offsite_task_read_uv_dedup, -- 站外任务期阅读UV(去重)
...
LEFT JOIN ods_xhs_grass_ad_conversion_di a
    ON GET_JSON_OBJECT(b.raw_data, '$.advertiser_id') = a.advertiser_id
    AND GET_JSON_OBJECT(b.raw_data, '$.creativity_id') = a.creativity_id
    AND b.grass_alliance = a.grass_alliance
    AND b.attribution_period = a.attribution_period
    AND b.dt = a.dt
    AND a.ds = '${bizdate}'
WHERE b.ds = '${bizdate}'
    -- 过滤所有指标均为空的行
    AND NOT (
        a.offsite_active_uv IS NULL
        AND a.offsite_task_cost IS NULL
        AND a.offsite_task_read_uv IS NULL
        AND a.offsite_active_uv_dedup IS NULL
        AND a.offsite_task_cost_dedup IS NULL
        AND a.offsite_task_read_uv_dedup IS NULL
        AND GET_JSON_OBJECT(b.raw_data, '$.read_uv') IS NULL
