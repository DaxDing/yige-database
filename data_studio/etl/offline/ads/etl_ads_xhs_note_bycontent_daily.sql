...
SELECT /*+ MAPJOIN(attr) */
    ...
    CAST(COALESCE(e.pgy_actual_amt, 0) AS BIGINT)                                   AS kols_fee,

    -- ============ 笔记数 ============
    1                                                                              AS note_count,

    -- ============ 曝光-点击指标 ============
    CAST(COALESCE(e.impression, 0) AS BIGINT)                                      AS impression,
    CAST(COALESCE(e.click, 0) AS BIGINT)                                           AS read_uv,

    -- ============ 互动指标 ============
    CAST(COALESCE(e.interaction, 0) AS BIGINT)                                     AS interaction,
    ...
    CAST(CASE WHEN attr.attribution_period = '15' THEN COALESCE(e.15d_shop_new_customer_uv, 0)
         ELSE COALESCE(e.30d_shop_new_customer_uv, 0) END AS BIGINT)             AS shop_new_customer_uv,
    -- 全店成交GMV
    ...
    -- ============ 搜索组件指标（表结构末尾） ============
    CAST(COALESCE(e.search_cmt_click, 0) AS BIGINT)                               AS search_cmt_click,
    -- 单品成交GMV
    ...
    CAST(COALESCE(e.search_cmt_impression, 0) AS BIGINT)                          AS search_cmt_impression,
