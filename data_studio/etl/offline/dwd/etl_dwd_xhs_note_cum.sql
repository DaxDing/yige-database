...
SELECT
    ...
    COALESCE(CAST(GET_JSON_OBJECT(o.raw_data, '$.engage_num') AS BIGINT), 0)
      - COALESCE(CAST(GET_JSON_OBJECT(o.raw_data, '$.like_num') AS BIGINT), 0)
      - COALESCE(CAST(GET_JSON_OBJECT(o.raw_data, '$.fav_num') AS BIGINT), 0)
      - COALESCE(CAST(GET_JSON_OBJECT(o.raw_data, '$.share_num') AS BIGINT), 0)
      - COALESCE(CAST(GET_JSON_OBJECT(o.raw_data, '$.cmt_num') AS BIGINT), 0)           AS follow,                  -- 关注(互动量残差)
    ...
    GETDATE() AS etl_time,
    -- 操作人字段
    GET_JSON_OBJECT(o.raw_data, '$.operate_user_id')                      AS operate_user_id,         -- 操作人ID
    GET_JSON_OBJECT(o.raw_data, '$.operate_user_name')                    AS operate_user_name,       -- 操作人名称
    -- 阅读量PV
    COALESCE(CAST(GET_JSON_OBJECT(o.raw_data, '$.read_num') AS BIGINT), 0)                AS read_num                 -- 阅读量PV
