-- ============================================================
-- ETL: ODS → DWD 计划层小时明细（离线报表）
-- 源表: ods_xhs_campaign_report_hi
-- 目标表: dwd_xhs_campaign_hi
-- 说明: 基于 etl_dwd_xhs_creative_hi 模板，标识字段替换为计划层维度
-- 差异: 计划层无 note_id/unit_id/creativity_id，新增 campaign_name
-- ============================================================

INSERT OVERWRITE TABLE dwd_xhs_campaign_hi PARTITION (ds='${bizdate}')
SELECT
    -- 标识字段
    a.campaign_id,                                                                          -- 计划ID
    GET_JSON_OBJECT(a.raw_data, '$.campaign_name')                       AS campaign_name,   -- 计划名称
    -- 策略字段
    ...
    -- 系统字段
    GETDATE() AS etl_time
FROM ods_xhs_campaign_report_hi a
WHERE a.ds = '${bizdate}'
;
