...
SELECT
    ...
    ABS(CAST(CAST(GET_JSON_OBJECT(a.raw_data, '$.order_amount') AS DECIMAL(18,2)) / 100 AS DECIMAL(18,2)))  AS cash_amount, -- 消费金额(分→元,取绝对值)
    -- 时间字段
    a.dt,                                                                                               -- 数据日期
    GETDATE()                                                                      AS etl_time          -- ETL时间
FROM ods_xhs_account_flow_di a
WHERE a.ds = '${bizdate}'
;
