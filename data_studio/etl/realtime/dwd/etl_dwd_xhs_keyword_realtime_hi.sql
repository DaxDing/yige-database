-- ============================================================
-- ETL: ODS → DWD 关键词层实时明细（按小时展开）
-- 源表: ods_xhs_keyword_realtime_hi
-- 目标表: dwd_xhs_keyword_realtime_hi
-- 说明: 通过 FROM_JSON + EXPLODE 将 hourly_data 数组展开为每小时一行
--       字段分组对齐 creative/campaign
-- ============================================================

INSERT OVERWRITE TABLE dwd_xhs_keyword_realtime_hi PARTITION (ds='${bizdate}')
SELECT
    -- 标识字段
    a.keyword_id,                                                                                             -- 关键词ID
    ...
    GET_JSON_OBJECT(a.raw_data, '$.base_keyword_dto.keyword')                             AS keyword,         -- 关键词
    -- 策略字段
    ...
    -- 系统字段
    GETDATE() AS etl_time
FROM ods_xhs_keyword_realtime_hi a
