# DataWorks PyODPS 节点：按项目校验创意实时数据完整性
import json
...
cherk_table = 'dwd_xhs_creative_realtime_hi'
cherk_source = '聚光创意层实时报表'
...
for row in projects:
    ...
    if base_count > cherk_count:
        ...
        LEFT JOIN {cherk_table} b
          ON a.creativity_id = b.creativity_id AND b.ds = '{ds}'
        WHERE a.ds = '{ds}' AND a.creativity_status = 'T'
          AND a.project_id = '{project_id}' AND b.creativity_id IS NULL
        """
