# DataWorks PyODPS 节点：按项目校验关键词维度表是否全部存在于 dwd_keyword，结果写入 MaxCompute
import json
from odps import options
options.sql.settings = {'odps.sql.type.system.odps2': 'true'}

ds = args['bizdate']
base_table = 'dim_xhs_keyword_df'
cherk_table = 'dwd_xhs_keyword_report_di'
cherk_source = '关键词层级离线报表'

# 1. 按项目统计基准数量 & 核对数量
count_sql = f"""
SELECT a.project_id,
       MAX(p.project_name) AS project_name,
       COUNT(DISTINCT a.keyword_id) AS base_count,
       COUNT(DISTINCT b.keyword_id) AS cherk_count
FROM {base_table} a
LEFT JOIN {cherk_table} b
  ON a.keyword_id = b.keyword_id AND b.ds = '{ds}'
...
for row in projects:
    ...
    if base_count > cherk_count:
        ...
        with o.execute_sql(miss_sql).open_reader() as r:
            missing = [row2['keyword_id'] for row2 in r]
