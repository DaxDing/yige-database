# DataWorks PyODPS 节点：按项目校验任务维度转化数据（星河/京准通 × 15/30天归因）
# 核对：DWD 全分区去重（转化数据按 dt 分区，不同任务组分布在不同 ds）
import json
...
base_table = 'dim_xhs_task_group_df'
cherk_table = 'ods_xhs_grass_bytask_conversion_df'
...
def run_check(alliance, period, cherk_source):
    ...
    SELECT tg.project_id,
           ...
           COUNT(DISTINCT tg.task_id) AS base_count,
           COUNT(DISTINCT b.task_id) AS cherk_count
    FROM {base_table} tg
    LEFT JOIN (
        SELECT DISTINCT task_id
        FROM {cherk_table}
        WHERE ds = '{ds}' AND grass_alliance = '{alliance}' AND CAST(attribution_period AS STRING) = '{period}'
    ) b ON tg.task_id = b.task_id
    LEFT JOIN dim_xhs_project_df p
      ON tg.project_id = p.project_id AND p.ds = '{ds}'
    WHERE tg.ds = '{ds}' AND tg.grass_alliance = '{alliance}'
    ...
    for row in projects:
        ...
        if base_count > cherk_count:
            ...
            WHERE tg.ds = '{ds}' AND tg.grass_alliance = '{alliance}'
              AND tg.project_id = '{project_id}' AND b.task_id IS NULL
            """
            with o.execute_sql(miss_sql).open_reader() as r:
                missing = [row2['task_id'] for row2 in r]
